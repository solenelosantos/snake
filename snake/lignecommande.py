...
def lignecommande():
    ...
    #check argument
    if not re.match(r"#[0-9A-Fa-f]{6}$", args.fc):
        raise ColorError(args.fc, "--fruit_color")
    if not re.match(r"#[0-9A-Fa-f]{6}$", args.hc):
        raise ColorError(args.hc, "--snake_head_color")
    if not re.match(r"#[0-9A-Fa-f]{6}$", args.bc):
        raise ColorError(args.bc, "--snake_body_color")
    return args